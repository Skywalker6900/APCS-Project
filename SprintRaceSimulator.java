public class SprintRaceSimulator {
    static class Athlete {
        ...
        public double getRandomizedTime(String event) {
            ...
            return baseTime * (0.95 + (Math.random() * 0.1)); // Randomize time within ±5% of personal record
        }
    }
    ...
    private static RaceResult[] simulateRace(Athlete[] athletes, String event) {
    ...
    private static void printResults(RaceResult[] results) {
        for (int i = 0; i < results.length; i++) {
            System.out.printf("%d. %-20s - %.2f seconds%n", 
                i + 1, 
                results[i].athlete, 
                results[i].time);
        }
